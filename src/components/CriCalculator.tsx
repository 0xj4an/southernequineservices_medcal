...
export default function CriCalculator({
  ...
  function resetDilutionState() {
    setShowDilution(false);
    setBagVolume(0);
    setDurationValue(12);
    setDurationUnit("hrs");
    setDiluent(DILUENTS[0]);
    setOutputUnit("ml/hr");
    setDropsPerMl(15);
  }
  ...
  function handleAddToProtocol() {
    if (!onAddToProtocol || !selectedMed) return;
    const parts: string[] = [];
    if (loadingDose > 0) {
  ...
  const durationHrs = toHours(durationValue, durationUnit);
  ...
  function renderDetailPanel() {
    if (!selectedMed || selectedLoadingDose === null) return null;
            ...
            <div>
              ...
              {selectedMed.rateMin === selectedMed.rateMax ? (
                ...
                <>
                  ...
                  <div className="flex gap-2 justify-center mt-2">
                    <button type="button" onClick={() => setSelectedRate(selectedMed.rateMin!)}
                    ...
                    <button type="button" onClick={() => setSelectedRate(parseFloat(((selectedMed.rateMin! + selectedMed.rateMax!) / 2).toFixed(4)))}
                      className="px-3 py-1 rounded-full text-xs font-medium border border-gray-300 hover:bg-[#c8a45a]/10 hover:border-[#c8a45a] transition-colors">
                      Mid
                    </button>
                    <button type="button" onClick={() => setSelectedRate(selectedMed.rateMax!)}
                      className="px-3 py-1 rounded-full text-xs font-medium border border-gray-300 hover:bg-[#c8a45a]/10 hover:border-[#c8a45a] transition-colors">
                      Max ({selectedMed.rateMax})
                    </button>
                  </div>
              ...
              <div className="mt-4">
                ...
                >
                  ...
                  <span className={`transition-transform ${showDilution ? "rotate-180" : ""}`}>
                    &#9662;
                  </span>
                </button>
              </div>
                  ...
                  <div className="mt-3 rounded-xl border border-gray-200 bg-gray-50 p-3 sm:p-4">
                    ...
                    <div className="mb-3">
                      ...
                      <div className="relative max-w-[12rem]">
                        <input type="number" min={0} max={10000} step={1} value={bagVolume}
                          onChange={(e) => { const val = parseInt(e.target.value, 10); if (!isNaN(val) && val >= 0) setBagVolume(val); }}
                      ...
                      <div className="flex flex-wrap gap-1 mt-1.5">
                        {COMMON_VOLUMES.map((vol) => (
                          <button key={vol} type="button" onClick={() => setBagVolume(vol)}
                            className={`px-2 py-0.5 rounded text-xs font-medium transition-colors ${bagVolume === vol ? "bg-[#c8a45a] text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>
                            {vol === 0 ? "None" : vol}
                          </button>
                        ))}
                      </div>
                    ...
                    {bagVolume > 0 && (
                      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                        <div>
                          ...
                          <div className="flex gap-1.5">
                            <div className="relative flex-1">
                              <input type="number" min={1} step={1} value={durationValue}
                                onChange={(e) => { const val = parseFloat(e.target.value); if (!isNaN(val) && val > 0) setDurationValue(val); }}
                        ...
                        <div>
                          ...
                          <select value={diluent} onChange={(e) => setDiluent(e.target.value)}
                            className="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm font-medium text-[#1a2332] focus:border-[#c8a45a] focus:outline-none focus:ring-2 focus:ring-[#c8a45a]/30">
                  ...
                  <div className="space-y-2 mt-3">
                    ...
                    <div className="bg-[#1a2332] rounded-xl px-4 py-3 text-center sm:px-6 sm:py-4">
                      ...
                      <div className="text-2xl font-black text-white sm:text-4xl">
                        ...
                        <span className="text-lg font-bold sm:text-2xl">{outputRate.label}</span>
                      </div>
                      <div className="flex justify-center gap-1.5 mt-2">
                        {OUTPUT_UNITS.map((u) => (
                          <button key={u} type="button" onClick={() => setOutputUnit(u)}
                            className={`px-2.5 py-1 rounded-full text-xs font-medium transition-colors ${outputUnit === u ? "bg-[#c8a45a] text-white" : "bg-white/10 text-gray-400 hover:bg-white/20"}`}>
                            {u}
                          </button>
                        ))}
                      </div>
                      {outputUnit === "drops/min" && (
                        <div className="flex items-center justify-center gap-1.5 mt-2">
                          {DROP_SETS.map((d) => (
                            <button key={d} type="button" onClick={() => setDropsPerMl(d)}
                              className={`px-2 py-0.5 rounded text-xs font-medium transition-colors ${dropsPerMl === d ? "bg-[#c8a45a] text-white" : "bg-white/10 text-gray-400 hover:bg-white/20"}`}>
                              {d} gtt/ml
                            </button>
                          ))}
                        </div>
  ...
  return (
    <div className="mx-auto max-w-6xl px-3 py-6 sm:px-6 sm:py-8 lg:px-8">
      {/* Medication Grid — detail panel renders inline below category */}
                  ...
                  >
                    <div className="mb-1 flex items-start justify-between gap-2">
                      ...
                      {isInProtocol ? (
                        ...
                        <span className={`inline-flex shrink-0 items-center rounded-full px-2 py-0.5 text-xs font-semibold ${
                          medHasRate ? "bg-blue-100 text-blue-800" : "bg-green-100 text-green-800"
                        }`}>
                    ...
                    <p className={`text-xs sm:text-sm ${isInProtocol ? "text-gray-400" : "text-gray-500"}`}>
                      {isInProtocol
                        ...
                        : `Loading: ${med.loadingDoseMin === med.loadingDoseMax ? `${med.loadingDoseMin} mg/kg` : `${med.loadingDoseMin} — ${med.loadingDoseMax} mg/kg`}`}
                    </p>
                    {!isInProtocol && medHasRate && (
                      <p className="text-xs text-gray-500 sm:text-sm">
                        Rate: {med.rateMin === med.rateMax ? `${med.rateMin}` : `${med.rateMin} — ${med.rateMax}`} {med.rateUnit}
                      </p>
                    )}
                    {!isInProtocol && (
